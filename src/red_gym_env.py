...
class RedGymEnv(Env):
    ...
    def __init__(
        ...
        self.log_every_n_steps = config['log_every_n_steps']      # print stats every n steps
        self.frame_every_n_steps = config['frame_every_n_steps']  # dump frame every n steps
        ...
        self.reset_count = 0
        self.step_count = 0 if 'step_count' not in config else config['step_count']
        self.clean_images_on_reset = True if 'clean_images_on_reset' not in config else config['clean_images_on_reset']
        self.all_runs = []

        # print(f'instance id: {self.instance_id} reset count: {self.reset_count} step count: {self.step_count}')
    ...
    def reset(self, seed=None, options=None):
        # print(f'resetting with seed {seed} at step {self.step_count}')
        ...
        # clean old curframe files
        if self.clean_images_on_reset:
            for f in Path(self.s_path).glob('curframe_*.jpeg'):
                f.unlink(missing_ok=True)

        # init screen exploration memory
        self.init_knn()
        self.init_map_mem()
    ...
    def append_agent_stats(self, action):
        ...
        self.agent_stats.append({
            'badge': self.get_badges(),                   # number of badges
            'coord_count': len(self.seen_coords),         # number of different coordinates seen (unused)
            'frames': self.knn_index.get_current_count(), # number of different frames seen
            'deaths': self.died_count,                    # number of deaths
            'event': self.progress_reward['event'],       # event reward
            'healr': self.total_healing_rew,              # total healing reward
            'hp': self.read_hp_fraction(),                # current hp fraction, read from memory
            'last_action': action,                        # last action taken
            'levels': levels,                             # level of each pokemon
            'levels_sum': sum(levels),                    # sum of levels of all pokemon
            'map': map_n,                                 # current map number
            'map_location': self.get_map_location(map_n), # current map name
            'pcount': self.read_m(PARTY_SIZE_ADDRESS),    # how many pokemon in party
            'ptypes': self.read_party(),                  # types of pokemon in party
            'step': self.step_count,                      # current step
            'total_reward': self.total_reward,            # total reward so far
            'x': x_pos,                                   # current x position
            'y': y_pos,                                   # current y position
        })
    ...
    def check_if_done(self):
        if self.early_stopping:
            return self.step_count > 128 and self.recent_memory.sum() < 255
        else:
            return self.step_count >= self.max_steps

    def save_and_print_info(self, done, obs_memory):
        should_log = self.step_count % self.log_every_n_steps == 0
        should_frame = self.step_count % self.frame_every_n_steps == 0

        if self.print_rewards and should_log:
            prog_string = f'step: {self.step_count:6d}'
            for key, val in self.progress_reward.items():
                prog_string += f' {key}: {val:5.2f}'
            prog_string += f' sum: {self.total_reward:5.2f}'
            print(f'{prog_string}', end='\n', flush=True)

        if should_frame:
            filename = self.s_path / Path(f'curframe_{self.instance_id}.jpeg')
            # print(f'saving file {filename}')
            plt.imsave(filename, self.render(reduce_res=False))
    ...
    def get_game_state_reward(self, print_stats=False):
        ...
        state_scores = {
            'event':     self.reward_scale*self.update_max_event_rew(),  # reward when events are triggered
            'level':     self.reward_scale*self.get_levels_reward(),     # reward for total party levels
            'heal':      self.reward_scale*self.total_healing_rew,       # reward for total healing
            'op_lvl':    self.reward_scale*self.update_max_op_level(),   # reward for max opponent level
            'badge':     self.reward_scale*self.get_badges() * 5,        # reward for each badge
            'explore':   self.reward_scale*self.get_knn_reward(),        # reward for exploration (frames/knn)
            'dead':      self.reward_scale*self.died_count * -0.1,       # penalty for dying
            #'party_xp':  self.reward_scale*0.1*sum(poke_xps),            # reward for total party xp
            #'op_poke':   self.reward_scale*self.max_opponent_poke * 800, # reward for max opponent pokemon party size
            #'money':     self.reward_scale* money * 3,                   # reward for total money
            #'seen_poke': self.reward_scale * seen_poke_count * 400,      # reward for seen pokemon
        }
